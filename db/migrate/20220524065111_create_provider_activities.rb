class CreateProviderActivities < ActiveRecord::Migration[6.1]
  def change
    table_base_structure = JSON.parse(File.read(Rails.root.join("db","structure","activities.json")))

    create_table :provider_activities do |t|
    ...
    add_reference   :provider_activities, :provider, foreign_key: true
