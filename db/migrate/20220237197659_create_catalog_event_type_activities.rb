class CreateCatalogEventTypeActivities < ActiveRecord::Migration[6.1]
  def change
    table_base_structure = JSON.parse(File.read(Rails.root.join("db","structure","activities.json")))

    create_table :catalog_event_type_activities do |t|
    ...
    add_reference   :catalog_event_type_activities, :catalog_event_types, foreign_key: true
