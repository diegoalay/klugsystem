class CreatePaymentMethodActivities < ActiveRecord::Migration[6.1]
  table_base_structure = JSON.parse(File.read(Rails.root.join("db","structure","activities.json")))

  create_table :payment_method_activities do |t|
  ...
  add_reference   :payment_method_activities, :payment_methods, foreign_key: true
