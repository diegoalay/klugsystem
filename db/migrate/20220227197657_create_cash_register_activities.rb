class CreateCashRegisterActivities < ActiveRecord::Migration[6.1]
  def change
    table_base_structure = JSON.parse(File.read(Rails.root.join("db","structure","activities.json")))

    create_table :cash_register_activities do |t|
    ...
    add_reference   :cash_register_activities, :cash_registers, foreign_key: true
