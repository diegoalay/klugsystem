class CreateProductTransactionTypeActivities < ActiveRecord::Migration[6.1]
  def change
    table_base_structure = JSON.parse(File.read(Rails.root.join("db","structure","activities.json")))

    create_table :product_transaction_type_activities do |t|
    ...
    add_reference   :product_transaction_type_activities, :product_transaction_type, foreign_key: true, index: { name: "product_transaction_type_activities_product_transaction_type" }
