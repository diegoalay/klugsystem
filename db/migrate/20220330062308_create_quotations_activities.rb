class CreateQuotationsActivities < ActiveRecord::Migration[6.1]
  def change
    table_base_structure = JSON.parse(File.read(Rails.root.join("db","structure","activities.json")))

    create_table :quotation_activities do |t|
    ...
    add_reference   :quotation_activities, :quotation, foreign_key: true
