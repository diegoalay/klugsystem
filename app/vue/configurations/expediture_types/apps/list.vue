<script type="text/javascript">
    export default {
        ...
        methods: {
            ...
            show(expediture_type){
                this.$router.push(this.url.config('expediture_types/:id', {id: expediture_type.id}).toString(false))
            },
            deleteRecord(id){
                const url = this.url.config('expediture_types/:id', {id: id})
                this.http.delete(url).then(result => {
                    if (result.successful) {
                        this.data = this.data.filter(e => e.id !== id)
                        this.pagination.total -= 1
                        this.$toast.success('Tipo de gasto eliminado exitosamente.')
                    } else {
